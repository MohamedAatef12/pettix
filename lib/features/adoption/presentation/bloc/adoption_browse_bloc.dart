...
class AdoptionBrowseBloc
  ...
  AdoptionBrowseBloc(this._getPagedPets, this._getPetOptions)
    ...
    on<SetDraftFilters>(_onSetDraft);
    on<ResetDraftFilters>(_onResetDraft);
    on<ApplyDraftFilters>(_onApplyDraft);
    on<OpenFilterSheet>(_onOpenFilterSheet);
  }
  ...
  // ─── Draft filter handlers (for stateless filter sheet) ─────────────────────

  void _onSetDraft(SetDraftFilters event, Emitter<AdoptionBrowseState> emit) {
    emit(
      state.copyWith(
        draftGenderId: event.genderId,
        draftSortBy: event.sortBy,
        draftSortDescending: event.descending,
      ),
    );
  }

  void _onResetDraft(
    ResetDraftFilters event,
    Emitter<AdoptionBrowseState> emit,
  ) {
    emit(
      state.copyWith(
        draftGenderId: null,
        draftSortBy: null,
        draftSortDescending: false,
      ),
    );
  }

  void _onOpenFilterSheet(
    OpenFilterSheet event,
    Emitter<AdoptionBrowseState> emit,
  ) {
    emit(
      state.copyWith(
        draftGenderId: state.selectedGenderId,
        draftSortBy: state.sortBy,
        draftSortDescending: state.sortDescending,
      ),
    );
  }

  Future<void> _onApplyDraft(
    ApplyDraftFilters event,
    Emitter<AdoptionBrowseState> emit,
  ) async {
    final genderId = state.draftGenderId;
    final sortBy = state.draftSortBy;
